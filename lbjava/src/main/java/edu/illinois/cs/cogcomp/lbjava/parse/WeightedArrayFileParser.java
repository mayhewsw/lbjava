...
/**
 * This parser returns an array of arrays representing each example. The first array represents the
 * integer keys of the example's features; the second array holds the values of those features. The
 * third array holds the example's label(s), and the fourth array holds the values of those labels.
 * These arrays are read in through files, and the paths to these files are passed in through the
 * constructor.
 *
 * <p>
 * When run as a stand-alone program, this class takes the names of example, lexicon, and model
 * files as input and prints all the feature vectors in the dataset to <code>STDOUT</code>.
 *
 * @author Michael Paul
 **/
@SuppressWarnings("ALL")
public class WeightedArrayFileParser implements Parser {
     ...
     **/
    public WeightedArrayFileParser(String exampleFile) {
        this(exampleFile, false);
    }
     ...
     **/
    public WeightedArrayFileParser(String exampleFile, boolean zip) {
     ...
     **/
    public WeightedArrayFileParser(byte[] data) {
        this(data, true);
    }
     ...
     * @param zip Whether or not the input stream is zipped.
     **/
    public WeightedArrayFileParser(byte[] data, boolean zip) {
        exampleData = data;
        zipped = zip;
        reset();
    }
    ...
    public void setIncludePruned(boolean b) {
        includePruned = b;
    }


    /**
     * Returns the number of examples left in the example file. This may be slow to compute as it
     * must read through the entire file and increment the count. {@link #reset()} is called after
    ...
    /**
     * Returns either an <code>Object[]</code> or a {@link FoldSeparator} deserialized out of the
     * given file.
     **/
    public Object next() {
    ...
    public void reset() {
        ...
        try {
            if (exampleFileName != null) {
                ...
                in = new DataInputStream(new ByteArrayInputStream(exampleData));
        } catch (Exception e) {
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            System.err
                    .println("usage: java edu.illinois.cs.cogcomp.lbjava.parse.ArrayFileParser <example file> <lexicon file> <lc file>");
            System.exit(1);
        }

        WeightedArrayFileParser parser = new WeightedArrayFileParser(exFileName);
        ...
        for (Object e = parser.next(); e != null; e = parser.next()) {
            FeatureVector v =
                    new FeatureVector((Object[]) e, learner.getLexicon(), learner.getLabelLexicon());
            v.sort();
